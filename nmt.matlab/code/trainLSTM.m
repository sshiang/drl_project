...
%   Hieu Pham: beam-search decoder.
  ...
  addOptional(p,'lstmSize', 200, @isnumeric); % number of cells, also embedding size
  ...
  addOptional(p,'numEpoches', 50, @isnumeric); % num epoches
  ...
  addOptional(p,'finetuneRate', 0.9, @isnumeric); % multiply learning rate by this factor each time we finetune
  ...
  addOptional(p,'minLenRatio', 0.2, @isnumeric);
  addOptional(p,'maxLenRatio', 3, @isnumeric);
