...
def irl(feature_matrix, n_actions, discount, MDP_states, transition_probability,
    ...
    for i in range(epochs):
        ...
        expected_svf = find_expected_svf(n_states, r, n_actions, discount, MDP_states,
...
def find_feature_expectations(feature_matrix, trajectories):
    ...
    for trajectory in trajectories:
        for state, action, new_state in trajectory:
            ...
            action = action.strip()
            MDP_state = (state, action, new_state)
            assert MDP_states.has_key(MDP_state)
            feature_expectations += feature_matrix[MDP_states[MDP_state]]

    feature_expectations /= float(len(trajectories))

    return feature_expectations

def find_expected_svf(n_states, r, n_actions, discount, MDP_states, 
                      transition_probability, trajectories):
    """
...
#    trajectory_length = trajectories.shape[1]
    ...
    for trajectory in trajectories:
        ...
        state = trajectory[0]
        action = trajectory[1].strip()
        new_state = trajectory[2][-1]
        start_state_count[MDP_states[(state, action, new_state)]] += 1
