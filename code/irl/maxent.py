...
def irl(feature_matrix, n_actions, discount, states, actions, MDP_states, transition_probability,
    ...
    MDP_n_states, d_states = feature_matrix.shape
    n_states = len(states)
    ...
    feature_expectations = find_feature_expectations(feature_matrix, states, actions, MDP_states, trajectories)

    # Gradient descent on alpha.
    for i in range(epochs):
        print("epoch {}".format(i))
        r = feature_matrix.dot(alpha)
        expected_svf = find_expected_svf(n_states, r, n_actions, discount, states, actions, MDP_states,
...
def find_feature_expectations(feature_matrix, states, actions, MDP_states, trajectories):
    """
    ...
    for trajectory in trajectories:
        for state, action, new_state in trajectory:
            new_state = new_state[-1]
            action = action.strip()
            MDP_state = MDP_states[(states[state], actions[action], states[new_state])]
            feature_expectations += feature_matrix[MDP_state]

    feature_expectations /= float(len(trajectories))

    return feature_expectations

def find_expected_svf(n_states, r, n_actions, discount, states, actions, MDP_states, 
                      transition_probability, trajectories):
    """
...
#    trajectory_length = trajectories.shape[1]
    ...
    policy = value_iteration.find_policy(n_states, n_actions, MDP_states, 
                                         transition_probability, r, discount)

    start_state_count = np.zeros(len(MDP_states))
    traj_len = 0
    for trajectory in trajectories:
        if len(trajectory) < 1:
            continue
        traj_len = max(traj_len, len(trajectory))
        start_traj = trajectory[0]
        state = start_traj[0]
        action = start_traj[1].strip()
        new_state = start_traj[2][-1]
        start_state_count[MDP_states[(states[state], actions[action], states[new_state])]] += 1
    p_start_state = start_state_count/n_trajectories

    expected_svf = np.tile(p_start_state, (traj_len, 1)).T
    temp_D = np.zeros(n_states)
    for t in range(1, traj_len):
        expected_svf[:, t] = 0
        for state in range(n_states):
            for action in range(n_actions):
                trans_state = (state, action)
                if transition_probability.has_key(trans_state) is False:
                    continue
                for new_state, prob in transition_probability[trans_state]:
                    target_state = MDP_states[(state, action, new_state)]
                    temp_D[new_state] += expected_svf[target_state, t-1]*policy[state, action]*prob
        for state in range(n_states):
            for action in range(n_actions):
                trans_state = (state, action)
                if transition_probability.has_key(trans_state) is False:
                ...
                for new_state, prob in transition_probability[trans_state]:
                    target_state = MDP_states[(state, action, new_state)]
                    expected_svf[target_state, t] = temp_D[state]*policy[state, action]*prob
        """
