...
def loadSource(source_without_padding):
    ...
    for s in source_without_padding:
        source.append([torch.FloatTensor(x).resize_(1,len(s[0])) for x in s])
    return s
...
def main():
    ...
    source_without_padding = loadPickle(opt.src)

    tgtF = open(opt.tgt) if opt.tgt else None
    for s in source_without_padding:
        #print("~~~: %d %d"%(len(s), len(s[0])))
        srcBatch += [torch.FloatTensor(x).resize_(1,len(x)) for x in s]
