...
def loadSource(source_without_padding):
    source = []
    for s in source_without_padding:
        source.append(torch.FloatTensor(s))
    return source
...
def main():
    ...
    source_without_padding = loadPickle(opt.src+".train.pkl")
    source = loadSource(source_without_padding)
    source_without_padding_valid = loadPickle(opt.src+".valid.pkl")
    source_valid = loadSource(source_without_padding_valid)

    featureSize = len(source_without_padding[0][0])

    trainData = onmt.Dataset(source,
                             dataset['train']['tgt'], opt.batch_size, opt.cuda)
    validData = onmt.Dataset(source_valid,
    ...
    if opt.train_from is None:
        encoder = onmt.Models.FeatureEncoder(opt, featureSize)
