...
class FeatureEncoder(nn.Module):
    ...
    def __init__(self, opt, featureSize):
        ...
        #self.state_lut = nn.Linear(featureSize, opt.word_vec_size, bias=False)
        #print(self.state_lut)
        #self.state_lut_activate = nn.Sigmoid() #opt.word_vec_size)
    ...
    def forward(self, input, hidden=None):
        ...
        batch_size = input.size(1) # batch first for multi-gpu compatibility
        #emb = self.state_lut_activate(self.state_lut(input)).transpose(0, 1)
        emb = input #torch.transpose(input, 0, 1)
        if hidden is None:
