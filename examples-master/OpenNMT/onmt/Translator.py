...
class Translator(object):
    ...
    def translateBatch(self, batch):
        ...
        for srcBatch_t in srcBatch.chunk(srcBatch.size(1), dim=1):
            #print("srcBatch_t: ")
            #srcBatch_t = (srcBatch_t_tensor.squeeze(dim=1)).squeeze(dim=0)
            #print(srcBatch_t.size())
            #print(srcBatch_t.squeeze(dim=0).size())
            #srcBatch_t = srcBatch_t.squeeze(dim=1)
